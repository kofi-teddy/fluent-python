# Convert 1D Array Into 2D Array

# You are given a 0-indexed 1-dimensional (1D) integer array original, 
# and two integers, m and n. You are tasked with creating a 2-dimensional 
# (2D) array with  m rows and n columns using all the elements from 
# original.
# The elements from indices 0 to n - 1 (inclusive) of original should form 
# the first row of the constructed 2D array, the elements from indices n 
# to 2 * n - 1 (inclusive) should form the second row of the constructed 
# 2D array, and so on.
# Return an m x n 2D array constructed according to the above procedure, or 
# an empty 2D array if it is impossible.


from typing import List


def construct_2D_array(original: List, m: int, n: int) -> List:
    # check if the total number of elements matches m * n
    if len(original) != m * n:
        return []
    
    result = []

    # Fill the 2D array by slicing the original array
    for i in range(m):
        row = original[i * n: (i + 1) * n]
        result.append(row)

    return result


original = [1, 2, 3, 4]
m, n = 2, 2

print(construct_2D_array(original, m, n))